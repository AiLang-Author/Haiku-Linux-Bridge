# Status for testers (2026-08-23, Day 55)
...
This page is the short public snapshot. Pickup / landmines for people hacking the trap: [`IMPLEMENTATION_PLAN.md`](IMPLEMENTATION_PLAN.md). Per-syscall table: [`SYSCALL_COVERAGE.md`](SYSCALL_COVERAGE.md). Latest wrap: [`STANDUP_DAY55.md`](STANDUP_DAY55.md). Punch-out: [`CLI_APPLET_PUNCHOUT.md`](CLI_APPLET_PUNCHOUT.md).

**Please test. File issues.** The CLI 90% set is wide enough that outside binaries will find the next holes faster than we will.

---

## What you can expect today (`main`, Day 55)
...
| `clone`/`fork` + `wait4` | `hello_fork` → `FORKOK`. `clone(CLONE_VM)` + child `exit(60)` `hello_clonevm` **`CLONEEXOK`**. SETTLS+CLEARTID **`CLONETLSOK`**. `CLONE_THREAD` + `wait4` `-ECHILD` `hello_clonethr` **`CLONETHROK`** (Day 55) |
...
| Interactive busybox **ash** | Terminal prompt, **`echo SHLIVE`** (Day 47–48). Stack pollfd **`POLLSTKOK`**. Ash fd 0 blocking still a tty stub |
...
from C (KDL). Stack pollfd is **`POLLSTKOK`**. Ash fd 0 blocking is
still a tty stub so `read()` gets keystrokes.
...
- **`CLONE_VM` / pthreads** — same-team `clone(CLONE_VM, stack)` + child `exit`(60) is **`CLONEEXOK`**. SETTLS/CLEARTID **`CLONETLSOK`**. `CLONE_THREAD` + `wait4(tid)` `-ECHILD` **`CLONETHROK`** (Day 55). glibc `pthread_create` may still miss `CLONE_FS|FILES|SIGHAND`.
