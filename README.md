...
- Raw `write` then exit lands in a redirect (`hello_wr` prints `WRPROBE`). glibc `puts` prints **`PRINTF_OK`**. busybox **`date` / `md5sum` / `nproc` write** on a redirected fd. Linux **TTY ioctl** maps onto Haiku tty (`WINSZ 25x80`). Linux **`/dev/fb0`** is Haiku VESA (**1280x800x32**). Interactive **busybox ash** on the Terminal: **`echo SHLIVE`**.
...
**Share this with testers:** [`docs/STATUS.md`](docs/STATUS.md) — what works, what to run, how to file a useful bug. Latest wrap: [`docs/STANDUP_DAY47.md`](docs/STANDUP_DAY47.md).
...
| Linux `poll` / `ppoll` | **Partial** — `nfds!=1` via `_user_wait_for_objects`. `nfds==1` no-copy stub (ash). Interactive ash **`echo SHLIVE`**. `hello_poll` `POLLOK` is a miss until a safe pollfd copy. Day 47. |
