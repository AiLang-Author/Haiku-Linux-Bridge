...
**Share this with testers:** [`docs/STATUS.md`](docs/STATUS.md) — what works, what to run, how to file a useful bug. Latest wrap: [`docs/STANDUP_DAY55.md`](docs/STANDUP_DAY55.md).
...
| Linux `clone` / `wait4` / `exit` | **`hello_fork` `FORKOK`.** `clone(CLONE_VM)` + child `exit`(60) **`CLONEEXOK`**. SETTLS + CLEARTID **`CLONETLSOK`**. `CLONE_THREAD` `wait4` `-ECHILD` **`CLONETHROK`**. See Day 21 / 51–55. |
