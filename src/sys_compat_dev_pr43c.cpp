 ...
 */
/* Static Linux ELF PT_LOAD lives near 0x400000. hello_poll's pfd
 * is .align 8 in .data. ash's stdin pollfd was 0x7fffffcfff01
 * (unmapped) and user_memcpy GPFd. Never copy that. */
static int
...
/* Driver memcpy is user_memcpy and GPFs on kernel pollfd temps
 * (iframe src 0x7fffffcfff01). No SMAP: a mapped VA is a mov. */
static void
...
sys_compat_poll(void* fds, int64 nfds, int64 timeoutMs, void* scratch)
{
	...
	if (nfds == 0)
		return 0;
	/*
	 * nfds==1 and pollfd not in ELF .data: ash. Do not copy.
	 * hello_poll / hello_pipeline pfd is in ELF .data.
	 */
	...
	if (sWaitObjFn == 0)
		return -LINUX_ENOSYS;
	/* No SMAP. user_memcpy of a mapped ELF pollfd still GPFd
	 * (iframe src 0x7fffffcfff01, dest 0x30301f) with rbx=0x403018.
	 * Direct memcpy of an area_for-checked VA. */
	if (area_for(ufds) <= 0)
		return -LINUX_EFAULT;
	kcopy(kfds, ufds, (int)nfds * 8);
	...
	if (area_for(scratch) <= 0)
		return -LINUX_EFAULT;
	kcopy(scratch, hinfos, (int)nfds * 8);
	...
	kcopy(hinfos, scratch, (int)nfds * 8);
