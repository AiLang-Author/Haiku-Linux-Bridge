...
#define AT_EXECFN 31
    ...
    *(--sp) = (uint64_t)linux_argc;
     ...
     */
/* ailang.x usage alone mmaps ~56MB ANON; a tiny compile ~629MB.
	 * Carve is munmap-no-op so size must cover the sum, not the peak. */
#define SYS_COMPAT_ARENA_SIZE (768u * 1024u * 1024u)
