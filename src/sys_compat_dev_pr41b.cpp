...
static uint8 sPollKfds[POLL_MAX_FDS * 8];
static uint8 sPollHinfos[POLL_MAX_FDS * 8];
...
{
	haiku_waitobj_fn fn;
	uint8* kfds = sPollKfds;
	uint8* hinfos = sPollHinfos;
	int32 i, n, ready;
	int32 fd;
	uint16 ev, rev;
	uint32 flags;
	...
	/* wait_for_objects on a Haiku tty GPFd (KDL). ash poll(0).
	 * fd 0/1/2 of an interactive Terminal are the tty — FIONREAD.
	 * Do not TCGETA here (that path still used user scratch). */
	if (nfds == 1) {
		...
		if (fd >= 0 && fd <= 2) {
			uint16 rev = 0;
			int64 r = poll_tty_fd(fd, ev, timeoutMs, &rev);
			kfds[6] = (uint8)rev;
			kfds[7] = (uint8)(rev >> 8);
