...
/* Import_ReadFile mmaps 64MB then munmap()s 64MB. Cap shrinks the
 * carve to 4MB; recycle must use the carved length or it overlaps. */
#define CAP_SLOTS 2048
static struct {
	uint64 addr;
	uint64 len;
} sCap[CAP_SLOTS];
...
static uint64
cap_take(uint64 a, uint64 fallback)
{
	int i;
	uint64 L;
	for (i = 0; i < CAP_SLOTS; i++) {
