...
sys_compat_poll(void* fds, int64 nfds, int64 timeoutMs, void* scratch)
{
	...
	a = (uint64)(addr_t)fds;
	/*
	 * 8-byte user_memcpy of an unaligned pollfd GPFd (src
	 * 0x7fffffcfff01, KDL). hello_poll's pfd is .align 8 in
	 * ELF .data. ash's stdin pollfd is not. Never copy the
	 * unaligned one. Never use gs:16-512 as wait scratch.
	 */
	if (!linux_user_ok(fds, (uint64)nfds * 8) || (a & 7) != 0) {
		if (nfds == 1) {
			if (timeoutMs == 0)
				return 0;
			snooze(5000);
	...
	if (nfds == 1) {
		fd = (int32)(kfds[0] | (kfds[1] << 8)
			| (kfds[2] << 16) | (kfds[3] << 24));
		ev = (uint16)(kfds[4] | (kfds[5] << 8));
		if (fd >= 0 && poll_fd_is_tty(fd)) {
			rev = 0;
			st = (int32)poll_tty_fd(fd, ev, timeoutMs, &rev);
			kfds[6] = (uint8)rev;
			kfds[7] = (uint8)(rev >> 8);
			if (user_memcpy(fds, kfds, 8) != B_OK)
				return -LINUX_EFAULT;
			return (st > 0) ? 1 : 0;
		}
	}
	...
	if (sWstatScratch < 0x100000ULL || sWaitObjFn == 0)
		return -LINUX_ENOSYS;
