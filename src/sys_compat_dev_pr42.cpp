...
/* area_for <= 0: VA is not in this team's aspace. Copying those
 * pollfds KDLd (GPF). Canonical-range alone is not enough. */
static int
user_area_ok(const void* p, uint64 n)
{
	uint64 a;
	int32 a0, a1;

	if (!linux_user_ok(p, n))
		return 0;
	a = (uint64)(addr_t)p;
	a0 = area_for((void*)(addr_t)a);
	if (a0 <= 0)
		return 0;
	if (n > 1) {
		a1 = area_for((void*)(addr_t)(a + n - 1));
		if (a1 <= 0)
			return 0;
	}
	return 1;
...
static int
copy_from_user(void* kdst, const void* usrc, uint64 n)
{
	uint64 i;

	if (n == 0)
		return 0;
	if (kdst == NULL || !user_area_ok(usrc, n))
		return -1;
	for (i = 0; i < n; i++) {
		if (user_memcpy((uint8*)kdst + i,
			(const uint8*)usrc + (uint32)i, 1) != B_OK)
			return -1;
	}
	return 0;
}
...
static int
copy_to_user(void* udst, const void* ksrc, uint64 n)
{
	uint64 i;

	if (n == 0)
		return 0;
	if (ksrc == NULL || !user_area_ok(udst, n))
		return -1;
	for (i = 0; i < n; i++) {
		if (user_memcpy((uint8*)udst + (uint32)i,
			(const uint8*)ksrc + i, 1) != B_OK)
			return -1;
	}
	return 0;
}
...
sys_compat_poll(void* fds, int64 nfds, int64 timeoutMs, void* scratch)
{
	...
	int32 wait_at[POLL_MAX_FDS];
	int32 i, nwait, ready, wi;
	int32 fd;
	uint16 ev, rev;
	uint32 flags;
	int64 tout;
	...
	if (nfds > POLL_MAX_FDS)
		return -LINUX_EINVAL;
	/*
	 * Copying ash's stdin pollfd with user_memcpy KDLd (GPF) when
	 * the VA was not in this team's aspace. Only copy mapped
	 * pollfds. nfds==1 fallback keeps interactive ash alive.
	 */
	if (!user_area_ok(fds, (uint64)nfds * 8)) {
		if (nfds == 1) {
			if (timeoutMs == 0)
				return 0;
			snooze(5000);
	...
	if (copy_from_user(kfds, fds, (uint64)nfds * 8) != 0)
		return -LINUX_EFAULT;
	...
	for (i = 0; i < (int32)nfds; i++) {
		...
		if (fd < 0)
			continue;
		/* wait_for_objects on a Haiku tty KDLd. FIONREAD. */
		if (poll_fd_is_tty(fd)) {
			if (tty_i < 0) {
				tty_i = i;
				tty_fd = fd;
				tty_ev = ev;
		...
		wi = nwait;
		wait_at[wi] = i;
		hinfos[wi * 8 + 0] = (uint8)fd;
		hinfos[wi * 8 + 1] = (uint8)(fd >> 8);
		hinfos[wi * 8 + 2] = (uint8)(fd >> 16);
		hinfos[wi * 8 + 3] = (uint8)(fd >> 24);
		{
			uint16 hev = linux_to_haiku_pevents(ev);
			hinfos[wi * 8 + 4] = 0; /* B_OBJECT_TYPE_FD */
			hinfos[wi * 8 + 5] = 0;
			hinfos[wi * 8 + 6] = (uint8)hev;
			hinfos[wi * 8 + 7] = (uint8)(hev >> 8);
		}
		nwait++;
	}
	...
	if (nwait > 0) {
		...
		if (sWstatScratch >= 0x100000ULL
			&& user_area_ok((void*)(addr_t)sWstatScratch,
				(uint64)nwait * 8))
			scratch = (void*)(addr_t)sWstatScratch;
		else if (!user_area_ok(scratch, (uint64)nwait * 8))
			return -LINUX_EFAULT;
		if (copy_to_user(scratch, hinfos, (uint64)nwait * 8) != 0)
			return -LINUX_EFAULT;
		if (timeoutMs < 0) {
			flags = 0;
		...
		st = fn(scratch, nwait, flags, tout);
		if (st < 0) {
			if ((uint32)(int32)st != 0x80000009
				&& (uint32)(int32)st != 0x8000000b)
				return haiku_status_to_linux((int64)st);
		} else {
			if (copy_from_user(hinfos, scratch,
				(uint64)nwait * 8) != 0)
				return -LINUX_EFAULT;
			for (wi = 0; wi < nwait; wi++) {
				uint16 hev = (uint16)(hinfos[wi * 8 + 6]
					| (hinfos[wi * 8 + 7] << 8));
	...
	if (tty_fd >= 0) {
		rev = 0;
		/* Timeout already spent on wait_for_objects. */
		if (nwait > 0)
			r = poll_tty_fd(tty_fd, tty_ev, 0, &rev);
		else
			r = poll_tty_fd(tty_fd, tty_ev, timeoutMs, &rev);
		if (r > 0 && tty_i >= 0) {
			kfds[tty_i * 8 + 6] = (uint8)rev;
			kfds[tty_i * 8 + 7] = (uint8)(rev >> 8);
			ready++;
		}
	}

	if (copy_to_user(fds, kfds, (uint64)nfds * 8) != 0)
		return -LINUX_EFAULT;
	return (int64)ready;
}
