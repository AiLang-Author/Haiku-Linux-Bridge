 ...
 */
/*
 * Driver memcpy is user_memcpy. user_memcpy of pollfd KDLd
 * (iframe src 0x7fffffcfff01) even when rbx was ELF pfd 0x403018.
 * No SMAP: a mapped VA is a MOV. gcc must not turn this into memcpy.
 */
...
{
	uint8 v;
	__asm__ __volatile__("movb (%1), %0" : "=q"(v) : "r"(p) : "memory");
	return v;
}
...
kstb(void* p, uint8 v)
{
	__asm__ __volatile__("movb %1, (%0)" :: "r"(p), "q"(v) : "memory");
}

static void
kcopy(void* dst, const void* src, int n)
{
	int i;
	for (i = 0; i < n; i++)
		kstb((uint8*)dst + i, kldb((const uint8*)src + i));
}

/* User VA with kernel GS GPFd (ELF .data 0x403018). uname
 * swapgs first. Use that for sWstatScratch too. */
__attribute__((noinline))
static uint8
kldb_user(const void* p)
{
	uint8 v;
	__asm__ __volatile__(
		"swapgs\n\t"
...
static void
kcopy_to_user(void* dst, const void* src, int n)
{
	int i;
	for (i = 0; i < n; i++)
		kstb_user((uint8*)dst + i, kldb((const uint8*)src + i));
}

static void
kcopy_from_user(void* dst, const void* src, int n)
{
	int i;
	for (i = 0; i < n; i++)
		kstb((uint8*)dst + i, kldb_user((const uint8*)src + i));
}

/* hello_poll pfd is .align 8 in ELF .data (~0x40xxxx). ash's stdin
 * pollfd was 0x7fffffcfff01. Never touch that. */
static int
...
sys_compat_poll(void* fds, int64 nfds, int64 timeoutMs, void* scratch)
{
	...
	if (nfds > POLL_MAX_FDS)
		return -LINUX_EINVAL;
	if (sWaitObjFn == 0 || sWstatScratch < 0x100000ULL)
		return -LINUX_ENOSYS;
	kser_putc('P');
	kser_hex((uint64)(addr_t)ufds);
	kser_putc('\n');
	/* Hook copied 8 bytes into gPollSnap with user GS (uname
	 * pattern). Do not kldb the user pollfd from C (that GPFd). */
	kcopy(kfds, &gPollSnap, 8);
	...
	kcopy_to_user(scratch, hinfos, (int)nfds * 8);
	...
	kcopy_from_user(hinfos, scratch, (int)nfds * 8);
