...
sys_compat_poll(void* fds, int64 nfds, int64 timeoutMs, void* scratch)
{
	...
	if (sWaitObjFn == 0)
		return -LINUX_ENOSYS;
	/* ash poll(0): copy 8 B to a kernel-stack slot (addon BSS
	 * as user_memcpy src KDLd). Do not wait_for_objects on tty. */
	if (nfds == 1) {
		uint8 one[8];
		uint16 rev;
		int64 r;
		...
		for (k = 0; k < 8; k++)
			one[k] = 0;
		if (user_memcpy(one, fds, 8) != B_OK)
			return -LINUX_EFAULT;
		fd = (int32)(one[0] | (one[1] << 8)
			| (one[2] << 16) | (one[3] << 24));
		ev = (uint16)(one[4] | (one[5] << 8));
		if (fd >= 0 && fd <= 2) {
			rev = 0;
			r = poll_tty_fd(fd, ev, timeoutMs, &rev);
			one[6] = (uint8)rev;
			one[7] = (uint8)(rev >> 8);
			if (user_memcpy(fds, one, 8) != B_OK)
				return -LINUX_EFAULT;
			return (r > 0) ? 1 : 0;
		}
		for (k = 0; k < 8; k++)
			kfds[k] = one[k];
	} else if (user_memcpy(kfds, fds, (size_t)nfds * 8) != B_OK)
		return -LINUX_EFAULT;
