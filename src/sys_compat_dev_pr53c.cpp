	...
	uint64 parentTid, uint64 childTid, uint64 tls)
{
	...
	uint8 tb[128];
	extern uint64 gCloneRdx, gCloneR8;
	uint8 attr[72];
	uint8 nameb[8];
	...
	/* movabs $parent_rdx,%rdx; movabs $parent_r8,%r8 */
	tb[n++] = 0x48;
	tb[n++] = 0xba;
	for (i = 0; i < 8; i++)
		tb[n++] = (uint8)(gCloneRdx >> (8 * i));
	tb[n++] = 0x49;
	tb[n++] = 0xb8;
	for (i = 0; i < 8; i++)
		tb[n++] = (uint8)(gCloneR8 >> (8 * i));
...
sys_compat_clone3(void* uargs, uint64 usize, uint64 userRip)
{
	...
	return sys_compat_clone_vm(userRip, childStack, a.flags,
