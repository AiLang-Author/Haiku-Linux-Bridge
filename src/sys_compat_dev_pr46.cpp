...
/* ELF .data (~0x40xxxx). Same swapgs movq as the poll hook.
 * C user_memcpy of a kernel dest is the driver memcpy trap. */
static void
put_user_u64(void* p, uint64 v)
{
	__asm__ __volatile__(
		"swapgs\n\t"
		"movq %1, (%0)\n\t"
...
sys_compat_poll(void* fds, int64 nfds, int64 timeoutMs, void* scratch)
{
	 ...
	 * timeout 0: kernel wait_for_objects_etc uses kernel=true
	 * (kernel io context) and reports READ on an empty pipe.
	 * Use the Linux write() flag instead.
	 * timeout != 0: _user_wait_for_objects (sWaitObjFn, syscall
	 * 0x06) with kernel=false so the team's pipe is the object.
	 * Pass the ELF pollfd pointer (0x40xxxx). sWstatScratch
	 * 0x7fffffcfff01 GPFd — do not use that.
	 */
	...
	if (sWaitObjFn == 0)
		return -LINUX_ENOSYS;
	/* Haiku object_wait_info over the ELF pollfd: object, type, events */
	hinfos[0] = (uint8)fd;
	hinfos[1] = (uint8)(fd >> 8);
	hinfos[2] = (uint8)(fd >> 16);
	hinfos[3] = (uint8)(fd >> 24);
	if (fd < 0) {
		hinfos[4] = 0;
		hinfos[5] = 0;
		hinfos[6] = 0;
		...
		hinfos[4] = 0; /* B_OBJECT_TYPE_FD */
		hinfos[5] = 0;
		hinfos[6] = (uint8)hev;
		hinfos[7] = (uint8)(hev >> 8);
	}
	put_user_u64(fds,
		(uint64)hinfos[0]
		| ((uint64)hinfos[1] << 8)
	...
	kser_puts("UB\n");
	st = fn(fds, (int32)nfds, flags, tout);
	kser_puts("US");
	kser_hex((uint64)(int64)st);
	kser_putc('\n');
	snap = get_user_u64(fds);
	hinfos[0] = (uint8)snap;
	hinfos[1] = (uint8)(snap >> 8);
	hinfos[2] = (uint8)(snap >> 16);
	hinfos[3] = (uint8)(snap >> 24);
	hinfos[4] = (uint8)(snap >> 32);
	hinfos[5] = (uint8)(snap >> 40);
	hinfos[6] = (uint8)(snap >> 48);
	hinfos[7] = (uint8)(snap >> 56);
	if (st < 0) {
		ready = 0;
		kfds[6] = 0;
		kfds[7] = 0;
	...
	rev = 0;
	if (fd >= 0) {
		uint16 hev = (uint16)(hinfos[6] | (hinfos[7] << 8));
		kser_puts("HE");
		kser_hex((uint64)hev);
		kser_putc('\n');
