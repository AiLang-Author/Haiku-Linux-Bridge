...
mmap_anon_haiku(uint64 len, int64 prot)
{
	...
	vr.address = NULL;
	vr.address_specification = B_RANDOMIZED_ANY_ADDRESS;
	vr._pad = 0;
	vr.alignment = 0;
	pr.low_address = 0;
	pr.high_address = 0;
	...
	int64 fd, int64 offset)
{
	...
	if ((flags & LINUX_MAP_ANON) != 0) {
		...
		/* 64MB Import_ReadFile buffers must not eat the carve.
		 * Haiku area is demand-paged; munmap uses linux_mmap. */
		if (len >= 65536 && sCreateAreaEtc != 0) {
			int64 a = mmap_anon_haiku(len, prot);
			if (a > 0)
				return a;
		}
