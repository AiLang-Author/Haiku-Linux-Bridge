#!/bin/sh
# PR53e: clone3 child SP%16==8 so glibc call *%rdx is ABI-aligned.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr53e.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr53e.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR53e' sys_compat_dev.cpp || echo 'NO_PR53E'
grep -n 'sCloneFn != 0' sys_compat_dev.cpp || echo 'NO_SP8'
...
chmod 755 /boot/home/hello_clonept /boot/home/hello_pthread /boot/home/run_pthread.sh
echo GO_CLONE53E_BUILT | tee /tmp/go_clone53e_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53e_built.txt "$HOST/results/go_clone53e_built.txt" || true
echo "=== clonept ==="
/boot/home/sys_compat_run /boot/home/hello_clonept; echo CLONEPT_RC=$?
sh /boot/home/run_pthread.sh
echo GO_CLONE53E_DONE | tee /tmp/go_clone53e_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53e_done.txt "$HOST/results/go_clone53e_done.txt" || true
echo GO_CLONE53E_DONE
