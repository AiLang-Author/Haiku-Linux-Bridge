#!/bin/sh
# PR45e: kernel wait_for_objects_etc; no _user_wait_for_objects.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr45e.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr45e.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR45e' sys_compat_dev.cpp || echo 'NO_PR45E'
...
echo GO_POLL45E_BUILT | tee /tmp/go_poll45e_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll45e_built.txt "$HOST/results/go_poll45e_built.txt" || true
sh /boot/home/run_poll.sh
echo GO_POLL45E_DONE | tee /tmp/go_poll45e_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll45e_done.txt "$HOST/results/go_poll45e_done.txt" || true
echo GO_POLL45E_DONE
