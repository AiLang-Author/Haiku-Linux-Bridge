#!/bin/sh
# PR54c: 2GB arena; compile ailang_cli.ailang on Haiku (self-compile).
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr54c.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr54c.S"
curl -s -o sys_compat_run.c "$HOST/src/sys_compat_run_pr54c.c"
for f in sys_compat_abi.h Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR54c' sys_compat_dev.cpp || echo 'NO_PR54C'
...
echo GO_SELFCC_BUILT | tee /tmp/go_selfcc_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_selfcc_built.txt "$HOST/results/go_selfcc_built.txt" || true
...
echo GO_SELFCC_DONE | tee /tmp/go_selfcc_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_selfcc_done.txt "$HOST/results/go_selfcc_done.txt" || true
echo GO_SELFCC_DONE
