#!/bin/sh
# PR53d: clone3/clone_vm on gs:8; pass glibc fn/arg in C.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr53d.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr53d.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR53d' sys_compat_dev.cpp || echo 'NO_PR53D'
grep -n 'CALL_GS8' syscall_hook.S || echo 'NO_GS8'
...
chmod 755 /boot/home/hello_pthread /boot/home/hello_clonept /boot/home/run_pthread.sh
echo GO_CLONE53D_BUILT | tee /tmp/go_clone53d_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53d_built.txt "$HOST/results/go_clone53d_built.txt" || true
echo "=== clonept ==="
/boot/home/sys_compat_run /boot/home/hello_clonept; echo CLONEPT_RC=$?
sh /boot/home/run_pthread.sh
echo GO_CLONE53D_DONE | tee /tmp/go_clone53d_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53d_done.txt "$HOST/results/go_clone53d_done.txt" || true
echo GO_CLONE53D_DONE
