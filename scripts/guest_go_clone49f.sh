#!/bin/sh
# PR49f: extra-thread _user_exit_thread + thread_exit; keep CR3 / skip exit_team.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr49f.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr49f.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR49f' sys_compat_dev.cpp || echo 'NO_PR49F'
grep -n '_user_exit_thread' sys_compat_dev.cpp || echo 'NO_UEXIT'
grep -n 'exit60_wait' syscall_hook.S || echo 'NO_WAIT'
...
echo GO_CLONE49F_BUILT | tee /tmp/go_clone49f_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone49f_built.txt "$HOST/results/go_clone49f_built.txt" || true
sh /boot/home/run_clonetls.sh
echo GO_CLONE49F_DONE | tee /tmp/go_clone49f_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone49f_done.txt "$HOST/results/go_clone49f_done.txt" || true
echo GO_CLONE49F_DONE
