#!/bin/sh
# PR53: clone3 + extra-thread futex wake for glibc pthread_join.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr53.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr53.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR53' sys_compat_dev.cpp || echo 'NO_PR53'
grep -n 'sys_compat_clone3' sys_compat_dev.cpp || echo 'NO_C3'
grep -n 'Lclone3' syscall_hook.S || echo 'NO_HOOK3'
...
echo GO_CLONE53_BUILT | tee /tmp/go_clone53_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53_built.txt "$HOST/results/go_clone53_built.txt" || true
sh /boot/home/run_pthread.sh
echo GO_CLONE53_DONE | tee /tmp/go_clone53_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53_done.txt "$HOST/results/go_clone53_done.txt" || true
echo GO_CLONE53_DONE
