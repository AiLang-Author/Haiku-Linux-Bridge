#!/bin/sh
# PR54x: carve exact mmap sizes from overcommit arena; no cap; no hook create_area.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s --max-time 30 -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr54x.cpp"
curl -s --max-time 30 -o syscall_hook.S "$HOST/src/syscall_hook_pr54x.S"
curl -s --max-time 30 -o sys_compat_run.c "$HOST/src/sys_compat_run_pr54x.c"
for f in sys_compat_abi.h Makefile.driver; do
	curl -s --max-time 15 -o "$f" "$HOST/src/$f"
done
grep -n 'PR54x' sys_compat_dev.cpp || echo 'NO_PR54X'
grep -n 'mmap_anon_carve' sys_compat_dev.cpp || echo 'NO_CARVE'
...
echo GO_PR54X_BUILT | tee /tmp/go_pr54x_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_pr54x_built.txt "$HOST/results/go_pr54x_built.txt" || true
echo GO_PR54X_BUILT
