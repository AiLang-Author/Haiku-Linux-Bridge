#!/bin/sh
# PR49g: extra-thread clear FS_BASE then _user_exit_thread + thread_exit.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr49g.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr49g.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR49g' sys_compat_dev.cpp || echo 'NO_PR49G'
grep -n 'FS0' sys_compat_dev.cpp || echo 'NO_FS0'
...
echo GO_CLONE49G_BUILT | tee /tmp/go_clone49g_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone49g_built.txt "$HOST/results/go_clone49g_built.txt" || true
sh /boot/home/run_clonetls.sh
echo GO_CLONE49G_DONE | tee /tmp/go_clone49g_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone49g_done.txt "$HOST/results/go_clone49g_done.txt" || true
echo GO_CLONE49G_DONE
