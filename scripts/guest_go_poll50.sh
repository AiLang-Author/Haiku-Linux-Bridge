#!/bin/sh
# PR50: nfds==1 pollfd copy from any aligned user pointer (stack + ELF).
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr50.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr50.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR50' sys_compat_dev.cpp || echo 'NO_PR50'
grep -n 'pollfd_user' sys_compat_dev.cpp || echo 'NO_USER'
...
echo GO_POLL50_BUILT | tee /tmp/go_poll50_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll50_built.txt "$HOST/results/go_poll50_built.txt" || true
sh /boot/home/run_pollstk.sh
echo GO_POLL50_DONE | tee /tmp/go_poll50_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll50_done.txt "$HOST/results/go_poll50_done.txt" || true
echo GO_POLL50_DONE
