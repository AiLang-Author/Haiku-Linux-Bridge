#!/bin/sh
# Rebuild driver with real chmod/chown/truncate/mprotect/uid/tid/prctl.
...
rm -f /boot/home/run_once_wstat /boot/home/run_once_ltp /boot/home/run_once_busy
SRC=/boot/home/src
mkdir -p "$SRC"
cd "$SRC"
...
curl -s -o /boot/home/run_wstat.sh "$HOST/scripts/guest_run_wstat.sh"
chmod 755 /boot/home/hello_min /boot/home/hello_wstat /boot/home/run_wstat.sh

echo "[+] dump Haiku syscall numbers"
gcc -O2 dump_sc.c -o /tmp/dump_sc && /tmp/dump_sc | tee /tmp/dump_sc.out
curl -s -X POST --data-binary @/tmp/dump_sc.out "$HOST/results/dump_sc_wstat.txt" || true
...
echo GO_WSTAT_DONE | tee /tmp/go_wstat_done.txt
curl -s -X POST --data-binary @/tmp/go_wstat_done.txt "$HOST/results/go_wstat_done.txt" || true
echo GO_WSTAT_DONE
