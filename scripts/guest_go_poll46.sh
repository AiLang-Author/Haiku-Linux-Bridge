#!/bin/sh
# PR46: blocking ELF poll(-1) via _user_wait_for_objects on the ELF pollfd.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr46.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr46.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR46' sys_compat_dev.cpp || echo 'NO_PR46'
grep -n 'put_user_u64' sys_compat_dev.cpp || echo 'NO_PUTU64'
...
echo GO_POLL46_BUILT | tee /tmp/go_poll46_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll46_built.txt "$HOST/results/go_poll46_built.txt" || true
echo GO_POLL46_REBOOT
sync
reboot
