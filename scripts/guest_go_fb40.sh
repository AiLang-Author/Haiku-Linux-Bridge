#!/bin/sh
# Rebuild PR40 fbdev trap. driverinstall re-hooks LSTAR.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr40.cpp"
for f in syscall_hook.S sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR40' sys_compat_dev.cpp || echo 'NO_PR40_IN_SRC'
grep -n 'open_fb_node' sys_compat_dev.cpp || echo 'NO_FB_IN_SRC'
...
curl -s -o /boot/home/run_fb.sh "$HOST/scripts/guest_run_fb.sh"
chmod 755 /boot/home/hello_fb /boot/home/run_fb.sh
echo GO_FB_DONE | tee /tmp/go_fb_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_fb_done.txt "$HOST/results/go_fb_done.txt" || true
echo GO_FB_DONE
