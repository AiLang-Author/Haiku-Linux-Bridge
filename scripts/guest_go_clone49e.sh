#!/bin/sh
# PR49e: extra-thread kill_thread(self) then hlt; OS.h exit_thread was a no-op.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr49e.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr49e.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR49e' sys_compat_dev.cpp || echo 'NO_PR49E'
grep -n 'kill_thread' sys_compat_dev.cpp || echo 'NO_KILL'
...
echo GO_CLONE49E_BUILT | tee /tmp/go_clone49e_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone49e_built.txt "$HOST/results/go_clone49e_built.txt" || true
sh /boot/home/run_clonetls.sh
echo GO_CLONE49E_DONE | tee /tmp/go_clone49e_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone49e_done.txt "$HOST/results/go_clone49e_done.txt" || true
echo GO_CLONE49E_DONE
