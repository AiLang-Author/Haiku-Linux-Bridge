#!/bin/sh
# PR53f: revert clone3 SP-8; dump TCB; SETTLS/gettid breadcrumbs.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr53f.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr53f.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR53f' sys_compat_dev.cpp || echo 'NO_PR53F'
...
echo GO_CLONE53F_BUILT | tee /tmp/go_clone53f_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53f_built.txt "$HOST/results/go_clone53f_built.txt" || true
echo "=== clonept ==="
/boot/home/sys_compat_run /boot/home/hello_clonept; echo CLONEPT_RC=$?
sh /boot/home/run_pthread.sh
echo GO_CLONE53F_DONE | tee /tmp/go_clone53f_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53f_done.txt "$HOST/results/go_clone53f_done.txt" || true
echo GO_CLONE53F_DONE
