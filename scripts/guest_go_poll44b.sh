#!/bin/sh
# PR44b: hook swapgs-copy of ELF pollfd; C never kldb user pollfd.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr44b.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr44b.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR44b' sys_compat_dev.cpp || echo 'NO_PR44B'
...
echo GO_POLL44B_BUILT | tee /tmp/go_poll44b_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll44b_built.txt "$HOST/results/go_poll44b_built.txt" || true
sh /boot/home/run_poll.sh
echo GO_POLL44B_DONE | tee /tmp/go_poll44b_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll44b_done.txt "$HOST/results/go_poll44b_done.txt" || true
echo GO_POLL44B_DONE
