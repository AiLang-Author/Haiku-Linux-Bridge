#!/bin/sh
# PR49b: CLEARTID writes 0; no futex from extra-thread exit (that hung).
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr49b.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr49b.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR49b' sys_compat_dev.cpp || echo 'NO_PR49B'
grep -n 'TC' sys_compat_dev.cpp || echo 'NO_TC'
...
echo GO_CLONE49B_BUILT | tee /tmp/go_clone49b_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone49b_built.txt "$HOST/results/go_clone49b_built.txt" || true
sh /boot/home/run_clonetls.sh
echo GO_CLONE49B_DONE | tee /tmp/go_clone49b_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone49b_done.txt "$HOST/results/go_clone49b_done.txt" || true
echo GO_CLONE49B_DONE
