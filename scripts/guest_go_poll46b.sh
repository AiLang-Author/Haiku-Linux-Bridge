#!/bin/sh
# PR46b: blocking ELF poll(-1) via write-flag + snooze. No _user_wait_for_objects.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr46b.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr46b.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR46b' sys_compat_dev.cpp || echo 'NO_PR46B'
grep -n 'sPollWrote' sys_compat_dev.cpp || echo 'NO_WFLAG'
...
echo GO_POLL46B_BUILT | tee /tmp/go_poll46b_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll46b_built.txt "$HOST/results/go_poll46b_built.txt" || true
sh /boot/home/run_pollblk.sh
echo GO_POLL46B_DONE | tee /tmp/go_poll46b_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll46b_done.txt "$HOST/results/go_poll46b_done.txt" || true
echo GO_POLL46B_DONE
