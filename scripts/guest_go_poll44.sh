#!/bin/sh
# PR44: pollfd copy via inline-asm movb. No memcpy/user_memcpy in poll.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr44.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr44.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR44' sys_compat_dev.cpp || echo 'NO_PR44'
grep -n 'kldb' sys_compat_dev.cpp || echo 'NO_KLDB'
...
echo GO_POLL44_BUILT | tee /tmp/go_poll44_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll44_built.txt "$HOST/results/go_poll44_built.txt" || true
sh /boot/home/run_poll.sh
echo GO_POLL44_DONE | tee /tmp/go_poll44_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll44_done.txt "$HOST/results/go_poll44_done.txt" || true
echo GO_POLL44_DONE
