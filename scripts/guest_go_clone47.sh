#!/bin/sh
# PR47: Linux clone(CLONE_VM) via Haiku spawn_thread in the same team.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr47.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr47.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR47' sys_compat_dev.cpp || echo 'NO_PR47'
grep -n 'clone_vm' syscall_hook.S || echo 'NO_CLONEVM'
...
echo GO_CLONE47_BUILT | tee /tmp/go_clone47_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone47_built.txt "$HOST/results/go_clone47_built.txt" || true
sh /boot/home/run_clonevm.sh
echo GO_CLONE47_DONE | tee /tmp/go_clone47_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone47_done.txt "$HOST/results/go_clone47_done.txt" || true
echo GO_CLONE47_DONE
