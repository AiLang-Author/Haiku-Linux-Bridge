#!/bin/sh
# PR53c: clone3 trampoline restores rdx/r8 for glibc call *%rdx.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr53c.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr53c.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR53c' sys_compat_dev.cpp || echo 'NO_PR53C'
grep -n 'gCloneRdx' syscall_hook.S || echo 'NO_RDX'
...
echo GO_CLONE53C_BUILT | tee /tmp/go_clone53c_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53c_built.txt "$HOST/results/go_clone53c_built.txt" || true
sh /boot/home/run_pthread.sh
echo GO_CLONE53C_DONE | tee /tmp/go_clone53c_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53c_done.txt "$HOST/results/go_clone53c_done.txt" || true
echo GO_CLONE53C_DONE
