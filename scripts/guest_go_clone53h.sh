#!/bin/sh
# PR53h: PR53g trap + clone3fn isolation. revert SP-8; dump TCB; no trampoline syscall (gSavedRsp).
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr53g.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr53g.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR53g' sys_compat_dev.cpp || echo 'NO_PR53G'
grep -n 'TCB0' sys_compat_dev.cpp || echo 'NO_TCB'
...
curl -s -o /boot/home/hello_clone3fn "$HOST/payload/tests/hello_clone3fn"
...
chmod 755 /boot/home/hello_clonept /boot/home/hello_clone3fn /boot/home/hello_pthread /boot/home/run_pthread.sh
echo GO_CLONE53H_BUILT | tee /tmp/go_clone53h_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53h_built.txt "$HOST/results/go_clone53h_built.txt" || true
echo "=== clonept ==="
/boot/home/sys_compat_run /boot/home/hello_clonept; echo CLONEPT_RC=$?
echo "=== clone3fn ==="
/boot/home/sys_compat_run /boot/home/hello_clone3fn; echo CLONE3FN_RC=$?
sh /boot/home/run_pthread.sh
echo GO_CLONE53H_DONE | tee /tmp/go_clone53h_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53h_done.txt "$HOST/results/go_clone53h_done.txt" || true
echo GO_CLONE53H_DONE
