#!/bin/sh
# PR45d: kernel wait_for_objects_etc; no _user_wait_for_objects.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr45d.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr45d.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR45d' sys_compat_dev.cpp || echo 'NO_PR45D'
...
echo GO_POLL45D_BUILT | tee /tmp/go_poll45d_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll45d_built.txt "$HOST/results/go_poll45d_built.txt" || true
sh /boot/home/run_poll.sh
echo GO_POLL45D_DONE | tee /tmp/go_poll45d_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll45d_done.txt "$HOST/results/go_poll45d_done.txt" || true
echo GO_POLL45D_DONE
