#!/bin/sh
# PR54s: Linux-style mmap — one Haiku area per mmap, kernel worker.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s --max-time 30 -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr54s.cpp"
curl -s --max-time 30 -o syscall_hook.S "$HOST/src/syscall_hook_pr54s.S"
curl -s --max-time 30 -o sys_compat_run.c "$HOST/src/sys_compat_run_pr54s.c"
for f in sys_compat_abi.h Makefile.driver; do
	curl -s --max-time 15 -o "$f" "$HOST/src/$f"
done
grep -n 'PR54s' sys_compat_dev.cpp || echo 'NO_PR54S'
grep -n 'mmap_worker' sys_compat_dev.cpp || echo 'NO_WORKER'
grep -n 'MWgo' sys_compat_dev.cpp || echo 'NO_MWGO'
...
echo GO_PR54S_BUILT | tee /tmp/go_pr54s_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_pr54s_built.txt "$HOST/results/go_pr54s_built.txt" || true
echo GO_PR54S_BUILT
