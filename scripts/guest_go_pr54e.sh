#!/bin/sh
# PR54e: 2GB arena; compile ailang_cli.ailang on Haiku (PR54e C-open + self-compile).
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr54e.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr54e.S"
curl -s -o sys_compat_run.c "$HOST/src/sys_compat_run_pr54e.c"
for f in sys_compat_abi.h Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR54e' sys_compat_dev.cpp || echo 'NO_PR54C'
...
echo GO_PR54E_BUILT | tee /tmp/go_pr54e_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_pr54e_built.txt "$HOST/results/go_pr54e_built.txt" || true
echo "=== PR54e C-open + self-compile ailang_cli.ailang ==="
...
echo GO_PR54E_DONE | tee /tmp/go_pr54e_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_pr54e_done.txt "$HOST/results/go_pr54e_done.txt" || true
echo GO_PR54E_DONE
