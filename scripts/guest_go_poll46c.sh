#!/bin/sh
# PR46c: blocking poll(-1) waits for a write after poll starts (not a stale flag).
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr46c.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr46c.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR46c' sys_compat_dev.cpp || echo 'NO_PR46C'
grep -n 'sPollWrote = 0' sys_compat_dev.cpp || echo 'NO_CLEAR'
...
curl -s -o /boot/home/run_pollblk.sh "$HOST/scripts/guest_run_pollblk.sh"
chmod 755 /boot/home/hello_poll /boot/home/hello_pollblk /boot/home/run_pollblk.sh
echo GO_POLL46C_BUILT | tee /tmp/go_poll46c_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll46c_built.txt "$HOST/results/go_poll46c_built.txt" || true
sh /boot/home/run_pollblk.sh
echo GO_POLL46C_DONE | tee /tmp/go_poll46c_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll46c_done.txt "$HOST/results/go_poll46c_done.txt" || true
echo GO_POLL46C_DONE
