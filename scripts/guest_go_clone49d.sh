#!/bin/sh
# PR49d: extra-thread exit_thread(status) then hlt; do not fall into exit_team.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr49d.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr49d.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR49d' sys_compat_dev.cpp || echo 'NO_PR49D'
...
echo GO_CLONE49D_BUILT | tee /tmp/go_clone49d_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone49d_built.txt "$HOST/results/go_clone49d_built.txt" || true
sh /boot/home/run_clonetls.sh
echo GO_CLONE49D_DONE | tee /tmp/go_clone49d_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone49d_done.txt "$HOST/results/go_clone49d_done.txt" || true
echo GO_CLONE49D_DONE
