#!/bin/sh
# PR53g: revert SP-8; dump TCB; no trampoline syscall (gSavedRsp).
...
echo GO_CLONE53G_BUILT | tee /tmp/go_clone53g_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53g_built.txt "$HOST/results/go_clone53g_built.txt" || true
echo "=== clonept ==="
/boot/home/sys_compat_run /boot/home/hello_clonept; echo CLONEPT_RC=$?
sh /boot/home/run_pthread.sh
echo GO_CLONE53G_DONE | tee /tmp/go_clone53g_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_clone53g_done.txt "$HOST/results/go_clone53g_done.txt" || true
echo GO_CLONE53G_DONE
