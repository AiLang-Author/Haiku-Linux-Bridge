#!/bin/sh
# PR54v: mmap worker, plain RW B_NO_LOCK (no STACK_AREA).
...
mkdir -p "$SRC"
cd "$SRC"
curl -s --max-time 30 -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr54v.cpp"
curl -s --max-time 30 -o syscall_hook.S "$HOST/src/syscall_hook_pr54v.S"
curl -s --max-time 30 -o sys_compat_run.c "$HOST/src/sys_compat_run_pr54v.c"
for f in sys_compat_abi.h Makefile.driver; do
	curl -s --max-time 15 -o "$f" "$HOST/src/$f"
done
grep -n 'PR54v' sys_compat_dev.cpp || echo 'NO_PR54V'
...
echo GO_PR54V_BUILT | tee /tmp/go_pr54v_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_pr54v_built.txt "$HOST/results/go_pr54v_built.txt" || true
echo GO_PR54V_BUILT
