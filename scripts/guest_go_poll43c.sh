#!/bin/sh
# PR43c: raw kcopy of ELF pollfd; no memcpy/user_memcpy in poll.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr43c.cpp"
...
grep -n 'PR43c' sys_compat_dev.cpp || echo 'NO_PR43C'
grep -n 'kcopy' sys_compat_dev.cpp || echo 'NO_KCOPY'
...
echo GO_POLL43C_BUILT | tee /tmp/go_poll43c_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll43c_built.txt "$HOST/results/go_poll43c_built.txt" || true
sh /boot/home/run_poll.sh
echo GO_POLL43C_DONE | tee /tmp/go_poll43c_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll43c_done.txt "$HOST/results/go_poll43c_done.txt" || true
echo GO_POLL43C_DONE
