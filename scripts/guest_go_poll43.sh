#!/bin/sh
# Rebuild PR43: copy pollfd only if it lives in ELF .data (0x400000-0x1000000).
# Hook no longer passes gs:16-512 as scratch (that pointer KDLd).
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr43.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr43.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR43' sys_compat_dev.cpp || echo 'NO_PR43_IN_SRC'
grep -n 'pollfd_in_elf' sys_compat_dev.cpp || echo 'NO_ELF_GATE'
grep -n 'xor	rcx, rcx' syscall_hook.S || echo 'NO_XOR_RCX'
...
echo GO_POLL43_BUILT | tee /tmp/go_poll43_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll43_built.txt "$HOST/results/go_poll43_built.txt" || true
sh /boot/home/run_poll.sh
echo GO_POLL43_DONE | tee /tmp/go_poll43_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll43_done.txt "$HOST/results/go_poll43_done.txt" || true
echo GO_POLL43_DONE
