#!/bin/sh
# PR54p: cap Import_ReadFile 64MB ANON to 4MB; 2GB arena; install only.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr54p.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr54p.S"
curl -s -o sys_compat_run.c "$HOST/src/sys_compat_run_pr54p.c"
for f in sys_compat_abi.h Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR54p' sys_compat_dev.cpp || echo 'NO_PR54P'
grep -n 'MC' sys_compat_dev.cpp || echo 'NO_CAP'
...
echo GO_PR54P_BUILT | tee /tmp/go_pr54p_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_pr54p_built.txt "$HOST/results/go_pr54p_built.txt" || true
echo GO_PR54P_BUILT
