#!/bin/sh
# PR43b: memcpy (not user_memcpy) of ELF .data pollfd after area_for.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr43b.cpp"
...
grep -n 'PR43b' sys_compat_dev.cpp || echo 'NO_PR43B'
...
echo GO_POLL43B_BUILT | tee /tmp/go_poll43b_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll43b_built.txt "$HOST/results/go_poll43b_built.txt" || true
sh /boot/home/run_poll.sh
echo GO_POLL43B_DONE | tee /tmp/go_poll43b_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_poll43b_done.txt "$HOST/results/go_poll43b_done.txt" || true
echo GO_POLL43B_DONE
