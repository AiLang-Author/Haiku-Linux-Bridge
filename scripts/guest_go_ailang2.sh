#!/bin/sh
# PR54b: 768MB arena + /proc/self/exe; run ailang.x on Haiku.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr54b.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr54b.S"
curl -s -o sys_compat_run.c "$HOST/src/sys_compat_run_pr54b.c"
for f in sys_compat_abi.h Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR54b' sys_compat_dev.cpp || echo 'NO_PR54B'
grep -n 'cmdline' sys_compat_run.c || echo 'NO_ARENA768'
...
echo GO_AILANG2_BUILT | tee /tmp/go_ailang2_built.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_ailang2_built.txt "$HOST/results/go_ailang2_built.txt" || true
echo "=== ailang usage ==="
...
echo GO_AILANG2_DONE | tee /tmp/go_ailang2_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_ailang2_done.txt "$HOST/results/go_ailang2_done.txt" || true
echo GO_AILANG2_DONE
