#!/bin/sh
# Restore Day 47 poll stub after PR44 KDL.
...
mkdir -p "$SRC"
cd "$SRC"
curl -s -o sys_compat_dev.cpp "$HOST/src/sys_compat_dev_pr41g.cpp"
curl -s -o syscall_hook.S "$HOST/src/syscall_hook_pr41g.S"
for f in sys_compat_abi.h sys_compat_run.c Makefile.driver; do
	curl -s -o "$f" "$HOST/src/$f"
done
grep -n 'PR41' sys_compat_dev.cpp || echo 'NO_PR41'
...
echo GO_SH41G_DONE | tee /tmp/go_sh41g_done.txt
curl -s --max-time 8 -X POST --data-binary @/tmp/go_sh41g_done.txt "$HOST/results/go_sh41g_done.txt" || true
echo GO_SH41G_DONE
